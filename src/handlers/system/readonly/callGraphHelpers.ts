/**
 * Small, SAP-independent decision helpers for GetCallGraph's callees
 * expansion — factored out of handleGetCallGraph.ts so they can be
 * unit-tested without a live ADT connection. The actual network calls that
 * consume their output stay thin wrappers in the handler.
 */
import type { CallGraphNode } from '../../../lib/callGraph';
...
export function isCalleeFetchableType(objectType: string): boolean {
 ...
 */
export function resolveFuncCalleeTarget(
  node: Pick<CallGraphNode, 'id' | 'object_type' | 'name'>,
  functionGroupOf: ReadonlyMap<string, string>,
): { functionModuleName: string; functionGroupName: string } | null {
  if (node.object_type !== 'FUNC') return null;
...
function rawTypePrefix(adtType: string): string {
 ...
 */
export function resolveNeighborObjectType(rawAdtType: string): string {
  return classifySourceType(rawAdtType) ?? rawTypePrefix(rawAdtType);
}
