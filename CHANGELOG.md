...
### Fixed
- **Data Preview cell-shift** — `parseSqlQueryXml` (shared by `GetSqlQuery` / `GetTableContents`) only matched paired `<dataPreview:data>...</dataPreview:data>` cells, so a self-closing empty cell (`<dataPreview:data/>`) produced no array entry and every later value in that column shifted up one row — real values from other rows landed in the wrong row, passing visual and schema inspection. Cells are now extracted in a single ordered pass matching both forms; self-closing cells become positional `null`s. Offline fixture tests cover mid-column self-closing (no shift), attribute-bearing self-closing, empty paired tags (`null`), and whitespace-only cells (preserved verbatim — CHAR initial values).
- **`CreateStructure` false success** — the handler ignored the required `fields`/`includes` input entirely (created with empty DDL; the update step was TODO-skipped) and still reported success, leaving a fieldless placeholder. It now generates DDIC `define structure` DDL from the input (new pure module `src/lib/structureDdl.ts`: data-element refs, built-in types with length/decimals, CURR/QUAN with `@Semantics` currency/unit reference via new optional `currency_reference`/`unit_reference` field attributes, includes, `@EndUserText.label`) and applies it through check→update under the create lock. Incomplete or inexpressible field specs fail with an explicit per-field error BEFORE any object is created — never a silent placeholder.
- **Structure lock-window kills the source PUT (live E2E)** — the DDL precheck ran between LOCK and the source PUT; any request inside that window ends the stateful ADT session and releases the lock, so the PUT failed with `ExceptionResourceInvalidLockHandle` (the same mechanism plausibly behind `UpdateStructure` never completing on live systems before). The check now runs BEFORE the lock in both `CreateStructure` and `UpdateStructure`, matching the proven include-handler pattern (lock → PUT → unlock, nothing in between). Additionally the DDL generator now always emits `@AbapCatalog.enhancement.category : #NOT_EXTENSIBLE` — without it the ADT source PUT is rejected with "Kein Sichern wegen Fehler in Quelle" (A/B-verified). End-to-end verified live on S/4HANA 2021: a 4-field structure (data-element refs + CURR with `@Semantics.amount.currencyCode` reference) created, activated, and read back; `UpdateStructure` completed its full chain for the first time. Known remaining defect (backlog): `DeleteStructure` reports success without deleting an ACTIVE structure (no lock taken, response not verified).
- **`DeleteStructure` false success on ACTIVE structures (live E2E)** — the handler posted to the `/sap/bc/adt/deletion` framework without a lock and without verifying the response; on an ACTIVE structure that request is a no-op and the handler still reported success. Rewritten to the proven include-delete pattern: stateful LOCK on the object URI → `DELETE {uri}?lockHandle=...` (+`corrNr`) → **read-back verification** (success only when the follow-up read returns 404; a still-readable object returns an explicit error, never success). Live-verified on S/4HANA 2021: an activated structure that had survived two "successful" deletes was deleted and confirmed gone (`lock` → `delete` → `verify_gone`).
- **`UpdateStructure` empty error text** — when the ADT check run returns status `notProcessed` with no message text, the vendored client throws `Structure check failed: ` with empty detail. The handler now replaces that with an actionable message (status, confirmation the update was not applied, retry / abapGit ZIP+Pull hint).
- **`CreateFunctionGroup` unverified success on ambiguous 400s** — the "Kerberos library not loaded" / "Business partner" HTTP 400 branches assumed creation succeeded without checking. The handler now reads the group back: success (`verified:true`, `activated:false`) only when it actually exists; otherwise the original 400 text plus the failed verification returns as an error.

### Added
- **Inactive-version warning on FM reads** — `GetFunctionModule` (new `check_inactive` input, default **true**) and `ReadFunctionModule` (default **false**, opt-in — costs one extra ADT call) compare the active source against an existing inactive version when reading `version='active'`; on divergence the response carries a `warning` telling the caller to re-read `version='inactive'` before editing, preventing a pending edit from being silently overwritten. The check never fails or delays the main read (fully isolated, no retries).
- **Truncation-honest Data Preview responses** — `GetSqlQuery` / `GetTableContents` responses now include `returned_row_count` (rows actually parsed), `truncated` (row cap hit, or server-reported total exceeds returned), and `server_total_rows` (present only when the XML carries `<dataPreview:totalRows>`). Additive fields; existing `total_rows` unchanged — stops caches recording a server total as if fully fetched.

### Changed
- Tool descriptions for `GetFunctionModule` / `ReadFunctionModule` / `UpdateFunctionModule` (+`UpdateFunctionModuleLow`) / `UpdateStructure` / `CreateStructure` / `CreateFunctionGroup` / `ActivateObjects` / `CheckSyntax` / `GetSqlQuery` / `GetTableContents` now document live-verified pitfalls and recipes in-band: the `version='active'` read trap that silently discards an unactivated edit, `UpdateFunctionModule`'s sibling-FM postcheck semantics (the write persists as the inactive version even when the post-write check fails), the FUGR mass-activation recipe (function group + TOP include + every FM + `SAPL<fugr>` main in one run; never batch unrelated objects), `CheckSyntax` scope semantics (`object_type='program'` does not walk the include tree — validate a function group per-FM with `function_group_name`; raw-source probes run in a REPORT context without fixed-point arithmetic), and Data Preview SQL constraints (long joins / IN lists → HTTP 400; shorten aliases or use BETWEEN; sporadic 400s under concurrency succeed on retry). Description text reflects post-fix behavior of the tools fixed in this release.
