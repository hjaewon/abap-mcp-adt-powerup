...
function classifySourceType(rawType) {
    ...
    if (t === 'CLAS' || t.startsWith('CLAS/'))
        return 'CLAS';
    if (t === 'PROG' || t.startsWith('PROG/'))
        return 'PROG';
    if (t === 'INTF' || t.startsWith('INTF/'))
        return 'INTF';
    if (t === 'FUNC' || t === 'FUGR/FF')
        return 'FUNC';
    if (t === 'FUGR' || t.startsWith('FUGR/'))
        return 'FUGR';
    return undefined;
}
 ...
 */
async function fetchObjectSource(client, connection, objectType, objectName, logger) {
    ...
    if (code === 'FUNC') {
        return {
            source: null,
            skipReason: 'Function module source requires both function_module_name and function_group_name; not resolvable from object_name alone. Use object_type "FUGR" with the function group name to search the whole group instead.',
        };
    }
    try {
        switch (code) {
        ...
        logger?.warn(`GrepObjects/GrepPackages: could not fetch source for ${objectType} ${name}: ${error instanceof Error ? error.message : String(error)}`);
        return {
            source: null,
...
//# sourceMappingURL=objectSourceFetch.js.map