...
exports.handleGetCallGraph = handleGetCallGraph;
const abapDependencyScan_1 = require("../../../lib/abapDependencyScan");
const callGraph_1 = require("../../../lib/callGraph");
...
exports.TOOL_DEFINITION = {
    ...
    inputSchema: {
        ...
        properties: {
            ...
            function_group: {
                type: 'string',
                description: 'Function group name — required only when object_type is FUNC (function modules are addressed as GROUP|NAME).',
            },
            direction: {
                type: 'string',
                enum: ['callers', 'callees', 'both'],
                description: "'callers' = who uses the root (default), 'callees' = what the root calls, 'both' = both traversals merged into one graph.",
                default: 'callers',
            },
            depth: {
                type: 'number',
            ...
            max_nodes: {
                type: 'number',
                description: 'Global cap on total nodes in the returned graph (max 300). Default 100.',
                default: DEFAULT_MAX_NODES,
            },
            custom_only: {
                type: 'boolean',
                description: 'When true (default), only Z*/Y*//NAMESPACE/ custom objects are expanded further during traversal — standard SAP objects still appear as leaf nodes but are not traversed past. The root is always expanded regardless of this flag.',
                default: true,
            },
        },
...
function extractFunctionGroupFromUri(uri) {
    if (!uri)
        return undefined;
 ...
 */
function buildCallersExpander(client, functionGroupOf, customOnly) {
    return async (node) => {
        ...
        for (const ref of result.references) {
            ...
            if (refType === 'FUNC') {
                const group = extractFunctionGroupFromUri(ref.uri) ??
...
function buildCalleesExpander(client, connection, logger, customOnly, functionGroupOf) {
    return async (node) => {
        ...
        ].filter((n) => (0, callGraph_1.makeNodeId)(n.object_type, n.name) !== node.id);
        return { neighbors, expandable: true };
    };
}
async function handleGetCallGraph(context, args) {
    ...
    try {
        const objectType = String(args?.object_type ?? '')
            ...
            .toUpperCase();
        if (!objectType) {
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, 'object_type is required');
        }
        if (!ROOT_OBJECT_TYPES.has(objectType)) {
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, `object_type must be one of ${Array.from(ROOT_OBJECT_TYPES).join(', ')}`);
        }
        ...
        if (objectType === 'FUNC') {
            functionGroup = String(args?.function_group ?? '')
                ...
                .toUpperCase();
            if (!functionGroup) {
                throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, 'function_group is required when object_type is FUNC');
            }
        }
        const direction = args?.direction ?? 'callers';
        if (!['callers', 'callees', 'both'].includes(direction)) {
            throw new utils_1.McpError(utils_1.ErrorCode.InvalidParams, "direction must be 'callers', 'callees', or 'both'");
        }
        const depth = Math.max(1, Math.min(MAX_DEPTH, Math.trunc(Number(args?.depth) || DEFAULT_DEPTH)));
        const maxNodes = Math.max(1, Math.min(MAX_MAX_NODES, Math.trunc(Number(args?.max_nodes) || DEFAULT_MAX_NODES)));
        const customOnly = args?.custom_only !== false;
        const rootId = (0, callGraph_1.makeNodeId)(objectType, rootName);
        logger?.info(`GetCallGraph: root=${rootId} direction=${direction} depth=${depth} max_nodes=${maxNodes} custom_only=${customOnly}`);
        const client = (0, clients_1.createAdtClient)(connection, logger);
        const functionGroupOf = new Map();
        ...
        if (direction === 'callers')
            ...
            expander = calleesExpander;
        else
            expander = (0, callGraph_1.combineDirectionExpanders)(rootId, callersExpander, calleesExpander);
        const result = await (0, callGraph_1.runCallGraphBfs)({ object_type: objectType, name: rootName }, expander, { maxDepth: depth, maxNodes });
        return {
            isError: false,
            content: [
                {
        ...
        logger?.error('Failed to build call graph', error);
        return {
            isError: true,
            content: [
                {
...
//# sourceMappingURL=handleGetCallGraph.js.map