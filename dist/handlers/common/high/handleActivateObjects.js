...
exports.handleActivateObjects = handleActivateObjects;
    ...
    inputSchema: {
        ...
        properties: {
            objects: {
                type: 'array',
                minItems: 1,
                description: 'Objects to activate in one batch. Supply either explicit uri, or name+type (and parent_name for FUGR/FF, FUGR/I).',
                items: {
                    type: 'object',
                    properties: {
                        name: {
                        ...
                        type: {
                            type: 'string',
                            description: "ADT object type code, e.g. 'PROG/P' (program), 'PROG/I' (include), 'CLAS/OC' (class), 'FUGR/FF' (function module).",
                        },
                        uri: {
                            type: 'string',
                            description: 'Explicit ADT URI. When provided, overrides name-based URI resolution.',
                        },
                        parent_name: {
                            type: 'string',
                            description: 'Parent name — required for FUGR/FF (function group) and FUGR/I (function-group include).',
                        },
                    },
                    required: ['name'],
                },
            ...
            run_timeout_ms: {
                type: 'number',
                description: 'Max time to wait for the activation run to finish (runs endpoint only). Default 120000.',
            },
        },
        required: ['objects'],
    },
};
async function handleActivateObjects(context, args) {
    const { connection, logger } = context;
    try {
        if (!args?.objects ||
            !Array.isArray(args.objects) ||
            args.objects.length === 0) {
            return (0, utils_1.return_error)(new Error('Missing required parameter: objects (must be a non-empty array)'));
        }
        for (const obj of args.objects) {
            if (!obj?.name && !obj?.uri) {
                return (0, utils_1.return_error)(new Error('Each object must carry at least "name" (or "uri")'));
            }
        }
        const inputs = args.objects.map((o) => ({
            name: o.name,
            type: o.type ?? '',
            uri: o.uri,
            parent_name: o.parent_name,
        }));
        logger?.info(`ActivateObjects: ${inputs.length} object(s) → /sap/bc/adt/activation/runs`);
        ...
        return (0, utils_1.return_error)(new Error(`ActivateObjects failed: ${message}`));
    }
}
//# sourceMappingURL=handleActivateObjects.js.map